...
void ResultScene::Update()
{
	...
	if (m_moveInteerval > *m_maxFps * MAXMOVEINTERVALSEC)
	{
		...
		m_timeMat[0] = Math::Matrix::CreateTranslation(NUMSTARTPOS, sin(DirectX::XMConvertToRadians((float)m_angle[0])) * 50.0f, 0.0f);
		for (int i = 1; i < PlayTime::ptnum; i++)
		{
			...
			m_timeMat[i] = Math::Matrix::CreateTranslation(NUMSTARTPOS + i * NUMSLIDEPOS, sin(DirectX::XMConvertToRadians((float)m_angle[i])) * 50, 0.0f);
		}
	}
}
...
void ResultScene::Init(int _minuets, int _seconds)
{
	...
	if (m_bClear)
	{
		...
		m_timeTex[PlayTime::tm].Load(m_numFileName[tenm]);
		m_timeTex[PlayTime::om].Load(m_numFileName[onem]);
		m_timeTex[PlayTime::colon].Load("Texture/time/minuet.png");
		m_timeTex[PlayTime::ts].Load(m_numFileName[tens]);
		m_timeTex[PlayTime::os].Load(m_numFileName[ones]);
		ResetNumPos();

		m_clearTimeTex.Load("Texture/Result/time.png");
		m_clearTimeMat = Math::Matrix::CreateTranslation(-250.0f, 0.0f, 0.0f);
	}
}
...
void ResultScene::ResetNumPos()
{
	...
	{
		m_angle[i] = 0;
		m_timeMat[i] = Math::Matrix::CreateTranslation(NUMSTARTPOS + i * NUMSLIDEPOS, 0.0f, 0.0f);
	}
}
