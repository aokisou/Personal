...
#define RunSpeed 3.0f			//çUåÇèÛë‘ÇÃéû
#define Dmg 10					//í èÌçUåÇéûÇÃÉ_ÉÅÅ[ÉW
#define Dmg2 15					//âÒì]çUåÇéûÇÃÉ_ÉÅÅ[ÉW
#define AttackIntervalSec 1		//çUåÇä‘äu
#define MaxHP 50				//èâä˙HP

void Minotaur::Init(Math::Vector2 _pos)
{
	const int ImgSize = 96;		//ÉLÉÉÉââÊëúÉTÉCÉY
	const float Scale = 2.5f;	//ÉLÉÉÉâägëÂó¶
...
void Minotaur::Action()
{
	...
	if (!m_bAlive) //èÊÇ¡Ç©ÇÍÇÈÇÊÇ§Ç…ìñÇΩÇËîªíË
	{
		std::shared_ptr<Player> p = m_pOwner->GetPlayer();
...
void Minotaur::Update(float _scrollX)
{
	...
	if (m_pState->GetStateType() <= enemyDeath || m_pState->GetStateType() == enemyBossEntry)
	{
		if (m_bDmg)
		{
			const float EmyMaxDmgEfcCnt = 10.0f;	//ê‘Ç≠åıÇÈéûä‘
